...
inline constexpr double factors_sn[] = {
    0.0, 0.7430, 1.8498, 0.9551, 1.3486, 0.9941, 1.1983, 1.0050, 1.1318, 1.0069,
...
inline constexpr double factors_qn[] = {
    0.0, 0.3995, 0.9939, 0.5133, 0.8441, 0.6122, 0.8589, 0.6700, 0.8736, 0.7201,
...
inline double get_sn_factor(size_t n) {
  ...
  if (n % 2 == 1)
    return 1.0 + 0.707 / n - 7.181 / (static_cast<double>(n) * n);
  else
    return 1.0 + 0.043 / n - 6.288 / (static_cast<double>(n) * n);
}
...
inline double get_qn_factor(size_t n) {
  ...
  if (n % 2 == 1)
    return 1.0 - 1.594 / n + 3.22 / (static_cast<double>(n) * n);
  else
    return 1.0 - 3.672 / n + 11.087 / (static_cast<double>(n) * n);
}
