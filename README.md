...
- **Cache-Aware Hybrid Architecture:** Six threshold parameters are derived from the CPU's L2 cache size (detected at install time via `sysctl`/`getconf`), controlling algorithm dispatch across three regimes:
  - **Micro-Scale ($N \le 128$ for $Q_n$; $N \le 2048$ for $S_n$):** Ultra-fast $O(n^2)$ exact brute-force kernel for $Q_n$ and stack-allocated fast-path for $S_n$.
...
- **Superior Accuracy:**
  ...
  - Consistent scale outputs verified against `robustbase` across all regimes.
...
Simply install and load. All hardware-aware calibrations happen silently and instantly.

```R
library(fastqnsn)
x <- rnorm(1e6)

# Automatically tuned for your specific CPU architecture
scale_sn <- sn(x)
scale_qn <- qn(x)
```
...
|:----------------------|:-----------------------------------------------|:---------|
| macOS (Apple Silicon) | `sysctl -n hw.perflevel1.l2cachesize` (E-core) | 4 MB     |
