...
# Akinshin's reference implementation (as described in the paper)
akinshin_sn <- function(x) {
  n <- length(x)
  if (n < 2) return(NA)
  factors <- c(NA, 0.7430, 1.8498, 0.9551, 1.3486, 0.9941, 1.1983, 1.0050, 1.1318, 1.0069, 
  ...
  predict_factor <- function(n) if (n %% 2 == 1) 1 + 0.707/n - 7.181/n^2 else 1 + 0.043/n - 6.288/n^2
  factor <- if (n <= 100) factors[n] else predict_factor(n)
  constant <- 1.19259855312321 * factor
  robustbase::Sn(x, constant = constant)
}

akinshin_qn <- function(x) {
  n <- length(x)
  if (n < 2) return(NA)
  factors <- c(NA, 0.3995, 0.9939, 0.5133, 0.8441, 0.6122, 0.8589, 0.6700, 0.8736, 0.7201, 
  ...
  predict_factor <- function(n) if (n %% 2 == 1) 1 - 1.594/n + 3.22/n^2 else 1 - 3.672/n + 11.087/n^2
  factor <- if (n <= 100) factors[n] else predict_factor(n)
  constant <- 2.21914446598508 * factor
  robustbase::Qn(x, constant = constant)
}

test_sizes <- c(10, 50, 150, 500, 1000)
results <- data.frame(n = integer(), estimator = character(), diff = numeric())
...
for (n in test_sizes) {
  ...
  # SN
  val_new_sn <- fastqnsn::sn(x)
  val_ref_sn <- akinshin_sn(x)
  diff_sn <- abs(val_new_sn - val_ref_sn)
  results <- rbind(results, data.frame(n = n, estimator = "Sn", diff = diff_sn))
  
  # QN
  val_new_qn <- fastqnsn::qn(x)
  val_ref_qn <- akinshin_qn(x)
  diff_qn <- abs(val_new_qn - val_ref_qn)
  results <- rbind(results, data.frame(n = n, estimator = "Qn", diff = diff_qn))
}

print(results)

if (all(results$diff < 1e-10)) {
  cat("\nSUCCESS: All tests passed with high precision!\n")
} else {
  cat("\nFAILURE: Significant discrepancies found!\n")
  quit(status = 1)
}
